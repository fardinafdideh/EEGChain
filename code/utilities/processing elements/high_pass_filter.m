function [ALLEEG, EEG, cfg] = high_pass_filter(ALLEEG, EEG, cfg, params)

global paramsGlobal

[resultsDir, dataName, cfg] = initialise(cfg, 'highpass'); % Initialise variables
...
%%
idx = find(strcmp(params, 'locutoff'));
EEG.comments = pop_comments(EEG.comments,'', ['High-pass filter applied: Cutoff ',num2str(params{idx + 1}),'Hz'],1);
...
saveas(h, fullfile(resultsDir, 'Highpass filter frequency response'), 'png');

%% Saving parameters
% required = {'locutoff', 1}; % - lower edge of the frequency pass band (Hz) {[]/0 -> lowpass}
...
%     };
% pathSave = '...\utilities\processing elements\low_pass_filter.mat';
